...
| BE-04 | Ma trận `truck_tables` | `getPriceMatrix`: mọi absolute `by_truck` trong book → **một** bảng (`truck_tables` 0 hoặc 1). Cột union `t:{label}:{unit}` first-seen (nhóm tỉnh+tên, bậc `sort_order`), Pallet cuối; ô thiếu `null`. Không `mergeCompatibleWeightBuckets`. Export `buildUnionTruckColumns` / `truckTierColumnKey` / `truckSchemaKey`. | M |
...
| FE-04 | `PriceMatrixTab` | Section **Theo loại xe** khi `truck_tables.length > 0` (kỳ vọng 1 bảng). Ẩn `schema_label`. Header `kind === 'truck'` `break-words`/`max-w`; cùng `visiblePeriods`. | M |
